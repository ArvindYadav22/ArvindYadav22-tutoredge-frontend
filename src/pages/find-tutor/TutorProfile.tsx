...
import Calendar from '../../components/findTutor/Calendar';
...
const TutorProfile: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '3rem auto', padding: 24, backgroundColor: '#fff', borderRadius: 8, boxShadow: '0 0 15px rgba(0,0,0,0.1)' }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 24 }}>
        ...
        <div>
          ...
          <p style={{ color: '#555', fontStyle: 'italic' }}>{tutorData.education}</p>
        </div>
      </div>
      ...
      <section style={{ marginBottom: 24 }}>
        <h3>Ratings</h3>
        <p style={{ fontWeight: 'bold', fontSize: 16 }}>{tutorData.rating} Stars</p>
        <div>
          {Object.entries(tutorData.ratingBreakdown).map(([star, percent]) => (
            <div key={star} style={{ display: 'flex', alignItems: 'center', marginBottom: 6 }}>
              ...
              <div style={{ height: 10, background: '#ddd', flexGrow: 1, borderRadius: 4, marginRight: 8 }}>
                <div style={{ width: `${percent}%`, background: '#007bff', height: '100%', borderRadius: 4 }} />
              </div>
              <div>{percent}%</div>
            </div>
          ))}
          ...
          {tutorData.subjects.map(subject => (
            <button
              ...
              style={{
                padding: '6px 14px',
                borderRadius: 20,
                border: selectedSubjects.includes(subject) ? '2px solid #007bff' : '1px solid #ccc',
                backgroundColor: selectedSubjects.includes(subject) ? '#cfe2ff' : 'transparent',
                cursor: 'pointer'
              }}
            >
              {subject}
          ...
          {tutorData.teachingModes.map(mode => (
            <button
              ...
              style={{
                padding: '6px 14px',
                borderRadius: 20,
                border: selectedMode === mode ? '2px solid #007bff' : '1px solid #ccc',
                backgroundColor: selectedMode === mode ? '#cfe2ff' : 'transparent',
                cursor: 'pointer'
              }}
            >
              {mode}
      ...
      <Button onClick={handleBooking} variant="primary" style={{ marginTop: 30, width: '100%' }}>
        Book a Demo
      </Button>
    </div>
  );
